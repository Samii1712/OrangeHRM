...
public class SearchAddedUSer {
	...
	public static void SearchAddedPIMUserRecord() {
		...
		String lastName = PIM.enteredLastName;

		WebElement nameInput = new WebDriverWait(BrowserSetup.driver, Duration.ofSeconds(10))
		...
		WebDriverWait wait = new WebDriverWait(BrowserSetup.driver, Duration.ofSeconds(5));
		wait.until(ExpectedConditions.visibilityOfElementLocated(suggestedList));
	...
	public static void captureRecord() {
     ...
     BrowserSetup.driver.executeScript("window.scrollBy(0,500)");
		...
		if (recordFound.equalsIgnoreCase("(1) Record Found")) {
			System.out.println("Record found: taking screenshot...");
